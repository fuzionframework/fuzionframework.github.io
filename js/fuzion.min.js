if(typeof(jQuery)==='undefined'){var jQuery;if(typeof(require)==='function'){jQuery=$=require('jquery')}else{jQuery=$}};if(typeof jQuery==='undefined'){throw new Error('Fuzion\'s JavaScript requires jQuery')}+function($){'use strict';var version=$.fn.jquery.split(' ')[0].split('.')
if((version[0]<2&&version[1]<9)||(version[0]==1&&version[1]==9&&version[2]<1)||(version[0]>3)){throw new Error('Fuzion\'s JavaScript requires jQuery version 1.9.1 or higher, but lower than version 4')}}(jQuery);+function($){'use strict';function transitionEnd(){var el=document.createElement('fuzion')
...
else $btn.find('input:visible,button:visible').first().trigger('focus')}}).on('focus.bs.button.data-api blur.bs.button.data-api','[data-toggle^="button"]',function(e){$(e.target).closest('.btn').toggleClass('focus',/^focus(in)?$/.test(e.type))})}(jQuery);$(document).ready(function(){$(".btn").click(function(event){$(this).blur()})});+function($){'use strict';var Carousel=function(element,options){this.$element=$(element)
...
if(options.hover){var open=!1;origin.unbind('click.'+origin.attr('id'));origin.on('mouseenter',function(e){if(open===!1){placeDropdown();open=!0}});origin.on('mouseleave',function(e){var toEl=e.toElement||e.relatedTarget;if(!$(toEl).closest('.dropdown-content').is(activates)){activates.stop(!0,!0);hideDropdown();open=!1}});activates.on('mouseleave',function(e){var toEl=e.toElement||e.relatedTarget;if(!$(toEl).closest('.dropdown-button').is(origin)){activates.stop(!0,!0);hideDropdown();open=!1}})}else{origin.unbind('click.'+origin.attr('id'));origin.bind('click.'+origin.attr('id'),function(e){if(!isFocused){if(origin[0]==e.currentTarget&&!origin.hasClass('active')&&($(e.target).closest('.dropdown-content').length===0)){e.preventDefault();if(options.stopPropagation){e.stopPropagation()}
placeDropdown('click')}
...
e.preventDefault()})})},select_tab:function(id){this.find('a[href="#'+id+'"]').trigger('click')}};$.fn.tabs=function(methodOrOptions){if(methods[methodOrOptions]){return methods[methodOrOptions].apply(this,Array.prototype.slice.call(arguments,1))}else if(typeof methodOrOptions==='object'||!methodOrOptions){return methods.init.apply(this,arguments)}else{$.error('Method '+methodOrOptions+' does not exist on jQuery.tooltip')}};$(document).ready(function(){$('ul.tabs').tabs()})}(jQuery));(function($){$.fn.tooltip=function(options){var timeout=null,margin=5;var defaults={delay:350,tooltip:'',position:'bottom',html:!1};if(options==="remove"){this.each(function(){$('#'+$(this).attr('data-tooltip-id')).remove();$(this).off('mouseenter.tooltip mouseleave.tooltip')});return!1}
...
if(menu_id.hasClass('fixed')){$(window).resize(function(){if(window.innerWidth>992){if($('#sidenav-overlay').length!==0&&menuOut){removeMenu(!0)}else{menu_id.css('transform','translateX(0%)')}}else if(menuOut===!1){if(options.edge==='left'){menu_id.css('transform','translateX(-100%)')}else{menu_id.css('transform','translateX(100%)')}}})}
...
menu_id.css('transform','translateX('+(x-options.menuWidth)+'px)')}else{if(x<(window.innerWidth-options.menuWidth/2)){menuOut=!0}
...
$('#sidenav-overlay').velocity({opacity:1},{duration:50,queue:!1,easing:'easeOutQuad'});dragTarget.css({width:'50%',right:0,left:''});menuOut=!0}else if(!menuOut||velocityX>0.3){$('body').css({overflow:'',width:''});menu_id.velocity({'translateX':[-1*options.menuWidth-10,leftPos]},{duration:200,queue:!1,easing:'easeOutQuad'});$('#sidenav-overlay').velocity({opacity:0},{duration:200,queue:!1,easing:'easeOutQuad',complete:function(){$(this).remove()}});dragTarget.css({width:'10px',right:'',left:0})}}else{if((menuOut&&velocityX>=-0.3)||velocityX>0.5){if(rightPos!==0){menu_id.velocity({'translateX':[0,rightPos]},{duration:300,queue:!1,easing:'easeOutQuad'})}
$('#sidenav-overlay').velocity({opacity:1},{duration:50,queue:!1,easing:'easeOutQuad'});dragTarget.css({width:'50%',right:'',left:0});menuOut=!0}else if(!menuOut||velocityX<-0.3){$('body').css({overflow:'',width:''});menu_id.velocity({'translateX':[options.menuWidth+10,rightPos]},{duration:200,queue:!1,easing:'easeOutQuad'});$('#sidenav-overlay').velocity({opacity:0},{duration:200,queue:!1,easing:'easeOutQuad',complete:function(){$(this).remove()}});dragTarget.css({width:'10px',right:0,left:''})}}}});$this.click(function(){if(menuOut===!0){menuOut=!1;panning=!1;removeMenu()}else{var $body=$('body');var oldWidth=$body.innerWidth();$body.css('overflow','hidden');$body.width(oldWidth);$('body').append(dragTarget);if(options.edge==='left'){dragTarget.css({width:'50%',right:0,left:''});menu_id.velocity({'translateX':[0,-1*options.menuWidth]},{duration:300,queue:!1,easing:'easeOutQuad'})}else{dragTarget.css({width:'50%',right:'',left:0});menu_id.velocity({'translateX':[0,options.menuWidth]},{duration:300,queue:!1,easing:'easeOutQuad'})}
...
var multiple=$select.attr('multiple')?!0:!1,lastID=$select.data('select-id');if(lastID){$select.parent().find('input').remove();$select.unwrap();$('ul#select-options-'+lastID).remove()}
...
$chips.data('chips',options.data);$chips.data('index',i);$chips.data('initialized',!0);if(!$chips.hasClass(self.SELS.CHIPS)){$chips.addClass('chips')}
...
value.done=!0}}}}}},100)}})(jQuery);(function(factory){if(typeof define=='function'&&define.amd)
...
else if(targetData.clear){P.clear().close(!0)}else if(targetData.close){P.close(!0)}})
...
view.addClass('initialized');pressed=!1;offset=target=0;images=[];item_width=view.find('.carousel-item').first().innerWidth();dim=item_width*2+options.padding;view.find('.carousel-item').each(function(i){images.push($(this)[0]);if(showIndicators){var $indicator=$('<li class="indicator-item"></li>');if(i===0){$indicator.addClass('active')}
...
function click(e){if(dragged){e.preventDefault();e.stopPropagation();return!1}else if(!options.full_width){var clickedIndex=$(e.target).closest('.carousel-item').index();var diff=(center%count)-clickedIndex;if(diff!==0){e.preventDefault();e.stopPropagation()}
